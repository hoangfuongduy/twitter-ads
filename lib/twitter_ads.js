...
function TwitterAdsAPI(options) {
	...
	self._validateConfigOrThrow = function (config) {
		if (config && typeof config !== 'object')
			throw new TypeError(
				'TwitterAdsAPI: Config must be object, got ' + typeof config
			);

		constants.CONFIGURABLE_KEYS.forEach(function (k) {
			if (config[k] !== undefined) self.config[k] = config[k];
			else if (constants.REQUIRED_KEYS.indexOf(k) > -1)
				throw new Error(
					util.format('TwitterAdsAPI: Config must include `%s`.', k)
				);
		});
	...
	self._makeRequest = function (type, url, params, body, cb) {
		if (!url)
			throw new Error(
				'TwitterAdsAPI: URL must be provided when making an API call.'
			);
		...
		if (!cb)
			throw new Error(
				'TwitterAdsAPI: Callback must be provided when making an API call.'
			);
		...
		var paramsClone = JSON.parse(JSON.stringify(params)),
			finalURL = Object.keys(paramsClone).length
				? helpers.moveParamsIntoPath(url, paramsClone)
				: url;

		paramsClone = helpers.normalizeParams(paramsClone);

		if (
			constants.JSON_PATHS.indexOf(url) > -1 &&
			['post', 'put'].indexOf(type) > -1
		) {
			request(
				{
					baseUrl:
						(self.config.sandbox
							? constants.API_SANDBOX_HOST
							: constants.API_HOST) +
						self.config.api_version +
						'/',
					url: Object.keys(paramsClone).length
						? finalURL + '?' + helpers.makeQueryString(paramsClone)
						: finalURL,
				...
				request.put(
					...
					function (err, resp, body) {
						...
						if (resp.statusCode !== 308 && resp.statusCode !== 201)
							nextChunk(
								new Error(
									'Twitter did not accept chunk upload, Should return status code 308 or 201 but instead returned: ' +
										resp.statusCode
								)
							);
						...
						if (resp.statusCode === 201)
							finalLocation =
								constants.TON_API_HOST +
								self.config.ton_api_version +
	...
	self._uploadTonFile = function (params, cb) {
		if (!params)
			throw new Error(
				'TwitterAdsAPI: Parameters must be provided when making TON API call.'
			);
		if (!cb)
			throw new Error(
				'TwitterAdsAPI: Callback must be provided when making an API call.'
		...
		constants.REQUIRED_TON_UPLOAD_PARAMS.forEach(function (k) {
			if (params[k] === undefined)
				throw new Error(
					util.format('TwitterAdsAPI: Parameters must include `%s`.', k)
				);
		});

		var defaultExpireTime = moment()
			.utc()
			.add(7, 'days')
			.toDate()
			.toUTCString();
		...
		request.post(
			...
			function (err, resp, body) {
				...
				if (resp.statusCode === 201) {
					...
					var minChunkSize = Number(
						resp.headers['X-TON-Min-Chunk-Size'.toLowerCase()]
					);
					return self._uploadTonChunks(
						fd,
						minChunkSize,
						urlPathForChunks,
						params,
					...
					return cb(
						new Error(
							'TwitterAdsAPI: Twitter TON API returned HTTP status code: ' +
								resp.statusCode
						)
					);
				}
...
TwitterAdsAPI.prototype.getOptions = function () {
	return this.config;
};
